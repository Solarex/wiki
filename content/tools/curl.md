...
- curl [options] [URL...]
    ...
    - FTP
        ...
        + ``curl --verbose ftp://ftp_server/ --user user``会看到curl执行了EPSV命令。``--eprt``参数显式指定使用EPRT模式。使用``--disable-epsv``选项禁用EPSV而使用PASV。使用``-P/--ftp-port <address>``选项指定使用EPRT，其中address参数可以指定为“-”，指定使用FTP控制连接相同的地.
